...
def calcoutparam(A_DATA, xgridsize, ygridsize):
    widnum = int((A_DATA.size[0] - 1) / xgridsize + 1)
    heinum = int((A_DATA.size[1] - 1) / ygridsize + 1)
...
def summarize(xgridsize, ygridsize, int_window, see_window, widnum, heinum,
    ...
    line = ["#", widnum, heinum, widnum * heinum, xgridsize, ygridsize, xposition[0], yposition[0]]
    csvform.append(line)
    line = ["#  xx", "   yy", " zz", "   uu", "   vv", " ww"]
    csvform.append(line)

    for j in range(int(heinum)):
        for i in range(int(widnum)):
...
def main():
    ...
    widnum, heinum = calcoutparam(A_DATA, xgridsize, ygridsize)

    xposition = [0 for i in range(widnum)]
    yposition = [0 for j in range(heinum)]
    xdisplacement = [[0 for i in range(widnum)] for j in range(heinum)]
    ydisplacement = [[0 for i in range(widnum)] for j in range(heinum)]
