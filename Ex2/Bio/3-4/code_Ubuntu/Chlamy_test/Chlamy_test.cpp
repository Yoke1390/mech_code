...
//標準入力にファイル名を入れる
// 例：実行ファイルと同じディレクトリにある動画ファイル (20240101_Left.avi) を処理したい場合
// >> ./Chlamy_test 20240101_Left.avi
...
{   
	...
	std::string dirname = filepath.substr(0,filepath.size()-4);//拡張子を含まないファイル名を取得
	mkdir(dirname.c_str(), 0777);//ディレクトリ作成
	chdir(dirname.c_str());//ディレクトリ移動
	
	// 画像処理したあとの動画ファイルを書き出すためのオブジェクトを宣言
	cv::VideoWriter writer;
	std::string writer_name="../Clone_" + dirname + ".mp4";//動画はプログラムと同じフォルダに保存する
	// 書き出し先の動画ファイル (writer) のプロパティを読み込み元 (videoまたはカメラキャプチャ) からコピーする    
	...
	fps    = video.get(cv::CAP_PROP_FPS);			// フレームレート
   	fourcc = cv::VideoWriter::fourcc('m', 'p', '4', 'v'); // ビデオフォーマットを指定(ISO MPEG-4 / .mp4)
	// 書き出し先の動画ファイル (writer) へ各値を設定する
	//writer.open(writer_name, fourcc, fps, cv::Size(width, height));


	// 3. 動体検知処理に使うオブジェクトなどを宣言する
	cv::Mat mat, image, frame, gray, gray32, avg32, delta32, thresh32, thresh;	//動体検知処理のためのオブジェクトを宣言
    	std::vector<std::vector<cv::Point>> contours;	//抽出された輪郭情報の格納先を宣言

    	//4. 動体検知
    	int count_frame=0;
    	char fname[30];
	while (count_frame<11) {
//	std::cout << "%d",count_frame << std::endl; 
		...
		// 4-3-2. 2フレーム目以降はavgを更新
	        cv::accumulateWeighted(gray32, avg32, 0.6);
        	
        	// 4-4. 現在 (gray32) と直近の過去 (image_avg) との差 (frame_Delta) を計算
		cv::absdiff(gray32, avg32, delta32);	
		
		// 4-5. deltaに閾値処理を行い、threshを得る
	        cv::threshold(delta32, thresh32, 0.03, 1, cv::THRESH_BINARY);
 		// 引数：inputpict outputpict, threshold, maxValue, 2値化の方法
        	...
        	// 引数：inputpict, 輪郭情報contours, CV_RETR_EXTERNAL 最も外側の輪郭のみを抽出, CV_CHAIN_APPROX_NONE すべての輪郭点を完全に格納
        	
        	// 4-7. 輪郭線を元のカラーフレーム画像imageに(青色で)描画する
       	cv::drawContours(image, contours, -1, cv::Scalar(255, 0, 0), 3);
       	// 引数：outputpict, contours, contourIdx=描かれる輪郭(負値の場合すべて), color
		...
		avg32.convertTo(avg32, CV_8U, 255);
		imwrite("2_avg.jpg", avg32);
		delta32.convertTo(delta32, CV_8U, 255);
		imwrite("3_delta.jpg", delta32);
		imwrite("4_threshold.jpg", thresh);
		imwrite("5_image_with_contour.jpg", image);

		}

		// 4-9. 画像 image を動画ファイルへ書き出す		
